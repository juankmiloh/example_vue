<template>
  <div class="transction-table">
    ...
    <el-row style="background:#fff;padding:16px 16px 0;margin-bottom:32px;">
      <el-col :key="name" :xs="24" :sm="24" :lg="24">
        <el-collapse>
    ...
    <el-table :data="format(data)" style="width: 100%;padding-top: 15px;" class="rcorners">
      ...
      <el-table-column label="Fecha Inicio" min-width="110" prop="date0" sortable>
        <template slot-scope="scope">
          {{ scope.row.date0 }}
        </template>
      </el-table-column>
      <el-table-column label="Precio" width="90" align="center" prop="value" sortable>
        <template slot-scope="scope">
          {{ scope.row.value0 }}
        </template>
      </el-table-column>
      <el-table-column label="Cantidad" width="110" align="center" prop="value" sortable>
        <template slot-scope="scope">
          {{ scope.row.value1 }}
        </template>
      </el-table-column>
  ...
  methods: {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['Id', this.itemTitle, this.descriptionTitle, 'Modalidad', 'Precio', 'Cantidad']
