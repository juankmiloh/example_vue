        ...
        <div class="chart-wrapper">
          <el-row :gutter="32">
            ...
            <el-col :key="name" :xs="24" :sm="12" :lg="6">
              <el-date-picker v-model="dateValue" type="monthrange" start-placeholder="Mes inicial" end-placeholder="Mes final" :placeholder="$t('datePicker.date')" @change="dateChange($event)" />
            </el-col>
  ...
  methods: {
    ...
    async fetchLatest() {
      try {
        ...
        this.dateValue = [new Date(range_data[0]), new Date(range_data[1])]
      } catch (error) {
        this.end = new Date()
        console.error(error)
      }
    ...
    async getData() {
      ...
      try {
        var result = await getIndicator(this.indicatorName, {
          fecha: this.formatDate(this.dateValue[0]),
          fecha_final: this.formatDate(this.dateValue[1]),
